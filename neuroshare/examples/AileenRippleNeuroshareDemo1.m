...
%function AileenRippleNeuroshareDemo1(varargin)
...
disp('Please load a .ns5 file, not .nev or .ns2')
...
AnalogSampleReadCount = min([AnalogEntityCount, MaxAnalogSampleCount]);
...
for i = 1:AnalogEntityCount
    ...
    [~, AnalogSampleCounts(i), AnalogWaveforms(:,i)] = ...
        ns_GetAnalogData(hFile, AnalogEntityIDs(i), ...
                         AnalogSampleReadStart, AnalogSampleReadCount);
end
fprintf('\n');
...
%clear hFile
%[ns_RESULT, SampleCount, Samples] = ns_GetAnalogData(hFile, AnalogEntityIDs(9), 1, AnalogItemCounts(9),'unscale');

[ns_RESULT, SampleCount, Samples] = ns_GetAnalogData(hFile,9,1,AnalogItemCounts(9),'unscale');\
plot(Samples)